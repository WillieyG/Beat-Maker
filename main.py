...
def draw_load_menu(index):
    ...
    exit_text = label_font.render('Close', True, white)
    screen.blit(exit_text, (WIDTH - 160, HEIGHT - 70))
    loaded_rectangle = pygame.draw.rect(screen, gray, [190, 90, 950, 500], 5, 5)
    ...
    for beat in range(len(saved_beats)):
        ...
        if 0 <= index < len(saved_beats) and beat == index:
            ...
            loaded_clicks_strings = saved_beats[beat][bpm_index_end + 14: -3]
            loaded_clicks_rows = list(loaded_clicks_strings.split('], ['))
